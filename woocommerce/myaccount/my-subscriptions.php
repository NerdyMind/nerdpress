...
<table class="shop_table my_account_subscriptions my_account_orders table table-striped table-hover table-bordered">
	...
	<?php foreach ( array_reverse( $subscriptions ) as $subscription_key => $subscription_details ) : ?>
		...
		<tr class="order">
			...
			<td class="subscription-actions order-actions text-center" data-title="<?php _e( 'Actions', 'woocommerce-subscriptions' ); ?>">
				<?php foreach( $actions[ $subscription_key ] as $key => $action ) : ?>
				<a href="<?php echo esc_url( $action['url'] ); ?>" class="btn btn-primary btn-sm <?php echo sanitize_html_class( $key ) ?>"><?php echo esc_html( $action['name'] ); ?></a>
				<?php endforeach; ?>
			</td>
		</tr>
	<?php endforeach; ?>
